...
struct BOMBlockTable {
    uint32_t numberOfBlockTablePointers; // Not all of them will be occupied. See header for number
                                         // of non-null blocks
...
struct BOMFreeList {
    ...
    BOMPointer freelistPointers[]; // mkbom adds two empty freelist pointers at the end and includes
                                   // this in header->indexLength
} __attribute__((packed));
